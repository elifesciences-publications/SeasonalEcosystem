%% m_e (2^entropy) vs c0
remap = load('Data/deterministic/2d_diversity_sweep_troph.mat');
D = remap.D;
P = (remap.chilist./(1+remap.chilist));
n0=remap.n0;
K=remap.K;
nSpecies=remap.m;
Ct = remap.diversity_c0_index;

determ_remap = load('Data/deterministic/2d_diversity_sweep.mat');
determ_Ct = determ_remap.diversity_c0_index;
determ_me = 2.^(determ_remap.D);

colors = cool(105);
fig3b = newfigure(3.42/2,3.42/2/1.3);
for i = 1:size(D,1)
    plot(Ct,2.^(D(i,:)), 'LineWidth', 1, 'Color', colors(round(P(i)*200),:)) 
    hold on
    c0eff = determ_Ct/(remap.chilist(i)+1)
end
set(gca,'FontSize',9);
...
text(10^(-7.1),39,'E', 'FontSize', 11, 'Interpreter', 'Latex');
...
print(gcf,['diversity_troph.png'],'-dpng','-r600');
print(gcf,['diversity_troph.eps'],'-depsc2');
