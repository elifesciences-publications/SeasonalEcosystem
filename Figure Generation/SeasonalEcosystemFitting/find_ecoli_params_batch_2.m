...
%of galactose-grown E coli in multisubstrate batch

%Import data, order: glucose galactose biomass
data = csvread('experimental_dynamics_2.csv');
...
% [~,batch_up_series] = simulate_ecoli_batch_2(xData,c_ints(1,2),c_ints(1,3),c_ints(1,1));
% [~,batch_low_series] = simulate_ecoli_batch_2(xData,c_ints(2,2),c_ints(2,3),c_ints(2,1));
...
rectangle('Position',[4.2,gal_y - (balance_offset-0)*offset - 0.2,1.5,(4/3)*2.1])
text(-1.3,3.1,'C','FontSize',11,'Interpreter','latex');

%Save
print(gcf,'-dpng','Figure_experimental_dynamics_2.png','-r300');
