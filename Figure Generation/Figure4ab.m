...
for SS=1:length(determ_spike.P)    
    loglog(determ_spike.diversity_c0_index, 2.^determ_spike.D(:,SS),'-','LineWidth',1,'Color',Smap(1+floor(determ_spike.P(SS)*200),:),'DisplayName',['S=' num2str(determ_spike.P(SS))]);
    hold on
end
loglog(determ_spike.diversity_c0_index, 21*ones(size(determ_spike.diversity_c0_index)),'--k','LineWidth',2,'DisplayName','m');
...
xlim(10.^[-3,3]);
xticks(10.^[-3,0,3]);
yld = ylabel('Eff. species $m_e$','Interpreter','latex','Units','normalized',...
    'FontSize', 9);
ylim([1, 32]);
...
box off
figfile = ['Diversity_collected_spike_0.05'];
set(gcf, 'Name', figfile);
text(10^(-4.82), 35,'A', 'FontSize', 11, 'Interpreter', 'Latex');
...
sigmamap = colormap(jet(length(determ_spike.bo)));
ha = tight_subplot(2,1,[0.03 0],[.312 0.04],[.25 0.1]);
to_plot = [6,1];
    
for tp=1:length(to_plot)
    % Get max strategy
    Esigma = sum(determ_spike.s,2);
    [maxE, mxsigma_ind] = max(Esigma);
    SS = to_plot(tp);
    axes(ha(tp));

    for sigma=1:length(determ_spike.s)
        loglog(determ_spike.diversity_c0_index, determ_spike.n0*determ_spike.blist(SS,:,sigma), '-', 'Color', sigmamap(sigma,:),'DisplayName',['\sigma=' num2str(sigma)]);
        hold on
        if(maxE>1 && sigma==mxsigma_ind)
            loglog(determ_spike.diversity_c0_index(end), determ_spike.n0*determ_spike.blist(SS,end,sigma), '*', 'Color', sigmamap(sigma,:));
        end
    end
    set(gca,'FontSize', 9);
    ...
    box off
    if(tp==1)
        text(10^(-5), 10^3.3,'B', 'FontSize', 11, 'Interpreter', 'Latex');
        xticklabels([]);
        xlabel('');
...
figfile = 'med_nsigma_spike_collected_spike_0.05';    
set(gcf, 'Name', figfile);
print(gcf,'-dpng',[figfile '.png'],'-r600');
print(gcf,'-depsc2',[figfile '.eps']);
