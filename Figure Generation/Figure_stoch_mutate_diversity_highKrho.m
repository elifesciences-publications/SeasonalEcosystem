% Plots stochstic simulation diversity for large K and large rho0
...
for ss=[1, length(S1s)]
    ...
    for cc=1:length(c0s)
        ...
        %             loglog(val/(N0s(NN)+c0s(cc)),'.-','Color',cmap(cc,:));
...
l.Position = [0.49192       0.5466       0.6859       0.4777];
l.Interpreter = 'Latex';
set(l.BoxFace, 'ColorType','truecoloralpha', 'ColorData',uint8(255*[0;0;0;0]));
l.Box = 'off';
l = line(10.^[0.48,1.51], 10.^[1, 1], 'LineWidth', 0.5, 'Color', Smap(1+floor(S1s(1)*200),:));
set(get(get(l,'Annotation'),'LegendInformation'), 'IconDisplayStyle','off') 
l = line(10.^[0.48,1.51], 10.^[-0.3, -0.3], 'LineWidth', 2, 'Color', Smap(1+floor(S1s(1)*200),:));
set(get(get(l,'Annotation'),'LegendInformation'), 'IconDisplayStyle','off') 
...
t0 = text(0.00135, 170,'$0$', 'Interpreter', 'Latex');
text(10^(-0.3), 170,'$\frac{1}{2}$', 'Interpreter', 'Latex');
c.Ticks=[];
...
box off
tA = text(1.7e-05,178,'A','FontSize',11, 'Interpreter', 'Latex');


print(gcf,'-dpng', 'FigStochHighkrho_composite.png', '-r600');
print(gcf,'-depsc2', 'FigStochHighkrho_composite.eps', '-r600');