function [b,Nr,Bstore,Cstore,tstore] = multispeciesbatchMADAPT_troph(bo,Bt,Ct,s,P,m,p,plt,K,chi)
...
rc = 1e-10; % remaining nutrients at end of batch (1e-8)
...
gr = @(Bvar,Cvar) Bvar.*(s*(Cvar./(K + Cvar)));

%SET CONSUMPTION FUNCTION--------------------------------------------------

if m > 1
    con = @(Bvar,Cvar) (eye(2) - chi)*((Cvar./(K + Cvar)).*transpose(sum(s.*repmat(Bvar,1,p))));
else
    con = @(Bvar,Cvar) (eye(2) - chi)*((Cvar./(K + Cvar)).*transpose(s.*repmat(Bvar,1,p)));
end
...
while u == 0
    ...
    relchange(~isfinite(relchange)) = 0;
    ...
    if max(C) < rc %Check for depletion below determined threshold
