function [b,Nr,Bstore,Cstore,tstore] = multispeciesbatchMADAPT(bo,Bt,Ct,s,P,m,p,plt,K)
...
rc = 1e-8; %Ratio of remaining nutrients at end of batch (1e-8)
...
while u == 0
    ...
    relchange = max(abs([gr1./B ; con1./C]));
    ...
    if max(C./Co) < rc %Check for depletion below determined threshold
